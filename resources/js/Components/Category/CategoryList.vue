                    ...
                    <div class="card-body">
                        ...
                        <div class="float-end">
                            <Link href="/CategorySavePage?id=0" class="btn btn-success mx-3 btn-sm">
                                Add Category <!-- if id=0 then add category, else update category -->
                            </Link>
                        </div>
...
<script setup>
    ...
    onMounted(() => {
        if (page.props.flash.message) {
            toaster.success(page.props.flash.message);   // Show success message as Category deleted successfully
        }
    });

    const DeleteClick = (id) => {
    let text = "Do you want to delete this category?";
    if (confirm(text) === true) {
        router.get(`/delete-category/${id}`);  // Route to delete category
    } else {
        text = "You canceled!";
    }
    };
